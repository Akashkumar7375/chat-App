 ...
 let handleImageVideoUpload=()=>{
  ...
  const handleform=(e)=>{
    ...
    if(messages.text|| messages.imageUrl || messages.videoUrl){
      ...
      formData.append('text',messages.text)
      formData.append('imageUrl',messages.imageUrl)
      formData.append('videoUrl',messages.videoUrl)
      if(sokectConnection){
        sokectConnection.emit('new message',{
          sender:user?._id,
          receiver:params?.id,
          formData:formData,
     ...
     <div className='flex flex-col gap-2 py-2 mx-2'  ref={currentMessage}>
      {
        allmessages.map((message,index)=>{
          ...
          <div key={index}  className={`bg-white p-1 py-1  rounded w-fit max-w-md  ${user._id === message.mesByUserId ? "ml-auto bg-teal-100":""}`}>
            <div className='w-full'>
              {message.imageUrl &&(
                <img src={URL.createObjectURL(message.imageUrl)}
